...
+ [调整] ComfyUI 部署套件重建镜像后、删除容器前再检查一次任务队列，取消时镜像和代码都退回旧版；没有终端时各项确认按默认值处理，不再等同 `-y`。
+ [新增] ComfyUI 部署套件菜单 2 更新前先预览新提交并标出风险；新增菜单 12「回到上次更新前」，更新后运行出问题时按旧依赖重建镜像。
+ [新增] ComfyUI 部署套件体检检查 torch 是否 CPU 版、前端等 comfy 包版本和源码要求是否一致、源码是否在分支上，服务不可用时 `--check` 返回非零；插件管理新增依赖预检（只看不装）；新增菜单 13「空间占用」，备份可选不含出图。
+ [调整] ComfyUI 部署套件在重启、重建、更新、恢复、改端口和改绑前检查队列，有任务在跑时先提示，默认不继续。
+ [修复] 画布部署脚本的数据说明改为指向应用内的 WebDAV 同步，不再说需要额外的同步方案。
+ [调整] ComfyUI 部署套件插件组改为汉化、APIimage、relayapi（gemini 走 chat 出图）和 rgthree，移除已归档的 Logic 和依赖本地模型的 Impact-Pack；Manager 改用官方 pip 包并以 `--enable-manager` 启用。
+ [修复] ComfyUI 部署套件修正反代 base_url 示例（不带 `/v1`）、模型/输出/输入目录挂载位置以及 `--update` 直接退出的问题；安装和更新时重建镜像并清理旧镜像，锁定 CPU 版 torch（pip 与 Manager 的 uv 都受约束）。
+ [新增] ComfyUI 部署套件菜单 5 改为「配 HTTPS + 访问密码」：生成带 basic_auth 的 Caddy 站点片段，并可把端口改绑到 Caddy 网关、公网只走域名。
+ [调整] ComfyUI 部署套件把 `data/user`（工作流、界面设置、Manager 配置）挂进容器并纳入一键备份；更新时源码或插件拉取失败会明确提示。
+ [修复] ComfyUI 部署套件自定义插件目录名拒绝带 `/` 或以 `.` 开头，卸载前二次确认；同端口重新部署不再误报端口被占；`--dir` 装到新目录会自动带上 Dockerfile；换目录恢复备份不再挂回旧目录的源码。
+ [新增] ComfyUI 部署套件新增菜单 11「测试反代」：从容器里探测 New API 连通性，可选列出令牌能用的出图模型；菜单 5 增加 Caddy 链路和域名密码自检。
+ [调整] ComfyUI 部署套件 `r` 改为 docker restart、`R` 重建容器；日志可看最近 200 行、实时跟随或只看本次启动的插件加载表和报错，并给容器加日志轮转；容器改用 SIGINT 停止，重启和停止不再等 10 秒超时。
+ [优化] ComfyUI 部署套件备份只留最近 5 份且只给自己读，恢复前校验备份包并保留本机端口和绑定方式；插件列表标出本地改动、卸载按序号选；更新时插件自动暂存本地改动，镜像构建失败自动退回源码和插件。
